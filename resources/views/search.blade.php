            ...
            @foreach ($products as $product)
                @if(!in_array($product->category->name,$categoriesFound))
                    <a role="button" class="btn btn-secondary"
                       href="{{$searchWord}}/{{$product->category->id}}">{{ $product->category->name }}</a>
                @endif
        ...
        <!-- Category Buttons -->
            ...
            @foreach ($products as $product)
                @if( $product->category->parent->name == $category->name )
                    @if(!in_array($product->category->name,$categoriesFound))
                        <a role="button" class="btn btn-secondary"
                           href="{{$product->category->id}}">{{$product->category->name}}</a>
                    @endif
                    <?php array_push($categoriesFound, $product->category->name);?>
                @endif
            @endforeach
                        ...
                        <div class="card-block p-3">
                            ...
                            <footer class="text-right">
                                ...
                                <a href="{{$product->category->id}}">
                                    <small class="badge badge-pill badge-info">{{ $product->category->name}}</small>
                                </a>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-block p-3">
                            ...
                            <footer class="text-right">
                                <small class="text-muted">{{ date('M-jS', strtotime($product->created_at)) }}</small>
                                <br/>
                                <a href="{{$searchWord}}/{{$product->category->id}}">
                                    <small class="badge badge-pill badge-info">{{ $product->category->name}}</small>
                                </a>
                                <small class="badge badge-pill badge-success">$ {{ $product->price }}</small>
                            </footer>
