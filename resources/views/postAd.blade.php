                            ...
                            @foreach ($categories as $category)
                                ...
                                @foreach ($category->children as $categoryTier2)
                                    ...
                                    @foreach ($categoryTier2->children as $categoryTier3)
                                        <option value="{{$categoryTier3->id}}">
                                            &emsp;&emsp;{{ $categoryTier3->name }}</option>
                                    @endforeach
                                @endforeach
                            @endforeach
                        </select>
            ...
            <strong>{{ $errors->first('title') }}</strong>
                    ...
                    <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
                        ...
                        <textarea class="form-control" rows="5" name="description" id="description"
                                  placeholder="Describe what you're selling... ">{{ old('description') }}</textarea>
          ...
          <strong>{{ $errors->first('image') }}</strong>
                    ...
                    <div class="checkbox">
                        <label><input type="checkbox" name="agree">
                            <small>I have read and agree to the <a href="#">Terms and Conditions</a></small>
                        </label>
                    </div>
